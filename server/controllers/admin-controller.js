...
async function addWorkspace(req, res, next) {
	const image = req.files['workspaceImage'][0].filename;
	...
	const rooms = [];
	for (let i = 0; req.body['rooms[' + i + '][label]']; i++) {
		rooms.push({
			label: req.body['rooms[' + i + '][label]'],
			image: req.files['rooms[' + i + '][image]'][0].filename,
			price: req.body['rooms[' + i + '][price]'],
		});
	}

	const amenities = [];
	for (let i = 0; req.body['amenities[' + i + '][label]']; i++) {
		amenities.push({
			label: req.body['amenities[' + i + '][label]'],
			image: req.files['amenities[' + i + '][image]'][0].filename,
